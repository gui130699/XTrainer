...
export function Providers({children}:{children:React.ReactNode}) { const [state,setState]=useState<State>({user:null,profile:null,loading:true}); useEffect(()=>{ const basePath = process.env.NEXT_PUBLIC_BASE_PATH ?? ""; if ("serviceWorker" in navigator) navigator.serviceWorker.register(`${basePath}/sw.js`).catch(()=>undefined); return onAuthStateChanged(auth, async user=>{ try { setState({user,profile:user ? await profile(user.uid) : null,loading:false}); } catch { setState({user,profile:null,loading:false}); } }); },[]); return <AuthContext.Provider value={state}>{children}</AuthContext.Provider>; }
